...
class ToTensor:
    ...
    def __call__(self, results):
        ...
        if 'masks' in self.keys:
            imgs = to_tensor(results['imgs'])
            masks = to_tensor(results['masks'])
            results['imgs'] = torch.cat((imgs, masks), dim=1)
            
            if 'label' in self.keys:
                results['label'] = to_tensor(results['label'])
        else:
            for key in self.keys:
                results[key] = to_tensor(results[key])
                
        del results['masks']        
        # removed_results = results.pop('masks')
        return results
    ...
    def __call__(self, results):
        ...
        if self.input_format == 'NCTHW':
            ...
            if 'masks' in results:
                ...
                masks = masks.reshape((-1, num_clips, clip_len) + masks.shape[1:])
                # N_crops x N_clips x L x H x W x C
                masks = np.transpose(masks, (0, 1, 5, 2, 3, 4))
                # N_crops x N_clips x C x L x H x W
