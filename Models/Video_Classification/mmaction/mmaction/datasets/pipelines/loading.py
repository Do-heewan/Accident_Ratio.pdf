    ...
    def __call__(self, results):
        ...
        for frame_idx in results['frame_inds']:
            ...
            if modality == 'RGB':
                ...
                filepath = osp.join(directory, img_list[frame_idx])
    ...
    def __call__(self, results):
        ...
        for frame_idx in results['frame_inds']:
            ...
            if modality == 'RGB':
                ...
                cur_frame = cv2.imread(filepath)
                
                if cur_frame is None:
                    cur_frame = np.zeros((1080,1920,3), np.uint8)
                else:    
                    cur_frame = cv2.resize(cur_frame, (1920,1080))
                    cur_frame = cv2.cvtColor(cur_frame, cv2.COLOR_BGR2RGB)
                #For classifier 1&2
                maskdir = directory.replace('image_jpg_new', 'image_mask') +'_mask12'

                maskpath = osp.join(maskdir, img_list[frame_idx])
                if(os.path.isfile(maskpath)):
                    curr_mask = cv2.imread(maskpath, cv2.IMREAD_GRAYSCALE)
                    if curr_mask is None:
                        curr_mask = np.zeros((1080,1920,1), np.uint8)
                    else:    
                        curr_mask = cv2.resize(curr_mask, (1920,1080))
    ...
    def __call__(self, results):
        ...
        for frame_idx in results['frame_inds']:
            ...
            if modality == 'RGB':
                ...
                #cur_frame = cv2.resize(cur_frame, (1920,1080))
                # For classifier 3
                maskdir = directory.replace('image_jpg_new', 'image_mask') +'_maskA'
    ...
    def __call__(self, results):
        ...
        for frame_idx in results['frame_inds']:
            ...
            if modality == 'RGB':
                ...
                maskdir = directory.replace('이미지', '이미지_mask') +'_maskB'
                maskpath = osp.join(maskdir, img_list[frame_idx])
                ...
                if curr_mask is None:
                    curr_mask = np.zeros((1080,1920), dtype = np.uint8)
