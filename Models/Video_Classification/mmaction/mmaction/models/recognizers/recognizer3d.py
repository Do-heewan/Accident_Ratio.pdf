...
class Recognizer3D(BaseRecognizer):
    ...
    def _do_test(self, imgs):
        ...
        # print("imgsdfdafadsasdasfadsadfs")
        # print(imgs)
        imgs = imgs.reshape((-1, ) + imgs.shape[2:])
        ...
        return cls_score, cls_pro

    def forward_test(self, imgs):
        """Defines the computation performed at every call when evaluation and
        # testing."""
        # output = self._do_test(imgs)
    ...
    def forward_test_with_constraint(self, imgs):
        """Defines the computation performed at every call when evaluation and
        testing."""
        cls_score, cls_pro = self._do_test(imgs)
        cls_score = cls_score.cpu().numpy()
        cls_pro = cls_pro.cpu().numpy()
        return cls_score, cls_pro
